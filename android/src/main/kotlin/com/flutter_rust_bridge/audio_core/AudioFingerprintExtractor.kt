...
object AudioFingerprintExtractor {
    ...
    fun extractFingerprint(context: Context, path: String): String? {
        val primary = runCatching {
            extractWithMediaCodec(context, path)
        }.getOrNull()
        if (primary != null) {
            return primary
        }

        NativeLog.w(TAG, "MediaCodec decode failed for fingerprint, falling back to ffmpeg")
        return ChromaprintNative.nativeGetFingerprintFromFileFfmpeg(path)
    }

    private fun extractWithMediaCodec(context: Context, path: String): String? {
