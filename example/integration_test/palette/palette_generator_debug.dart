...
class PaletteGenerator with Diagnosticable {
  ...
  static Future<PaletteGenerator> fromByteData(
    ...
    assert(
      region == null || (region.topLeft.dx >= 0.0 && region.topLeft.dy >= 0.0),
  ...
  /// Create a debug snapshot from encoded image [ByteData], width, and height.
  ///
  /// This is intended for parity debugging against non-Dart implementations.
  static Future<PaletteDebugSnapshot> debugFromByteData(
    EncodedImage encodedImage, {
    Rect? region,
    int maximumColorCount = _defaultCalculateNumberColors,
    List<PaletteFilter> filters = const <PaletteFilter>[
    ...
    final _QuantizerDebugResult quantizerDebug =
        await quantizer.quantizedColorsWithDebug;
    final PaletteGenerator generator = PaletteGenerator.fromColors(
      List<PaletteColor>.from(quantizerDebug.paletteColors),
      targets: targets,
    );

    return PaletteDebugSnapshot(
      quantizedHistogram: quantizerDebug.quantizedHistogram,
      filteredHistogram: quantizerDebug.filteredHistogram,
      paletteColors: List<PaletteColor>.from(generator.paletteColors),
      selectedSwatches: Map<PaletteTarget, PaletteColor>.from(
        generator.selectedSwatches,
      ),
      splitTrace: quantizerDebug.splitTrace,
    );
  }
  ...
  static Future<PaletteGenerator> fromImage(
    ...
    return PaletteGenerator.fromByteData(
      EncodedImage(imageData, width: image.width, height: image.height),
      region: region,
  ...
  static Future<PaletteGenerator> fromImageProvider(
    ...
    assert(
      region == null || size!.contains(region.topLeft),
  ...
  void _selectSwatches() {
    ...
    for (final PaletteTarget target in allTargets) {
      ...
      final PaletteColor? targetScore = _generateScoredTarget(
        target,
        usedColors,
      );
      if (targetScore != null) {
  ...
  PaletteColor? _generateScoredTarget(
    ...
    final PaletteColor? maxScoreSwatch = _getMaxScoredSwatchForTarget(
      target,
      usedColors,
    );
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      IterableProperty<PaletteTarget>(
        'targets',
        targets,
...
class PaletteColor with Diagnosticable {
  ...
  void _ensureTextColorsGenerated() {
    if (_titleTextColor == null || _bodyTextColor == null) {
      ...
      final int? lightBodyAlpha = _calculateMinimumAlpha(
        white,
        color,
        _minContrastBodyText,
      );
      final int? lightTitleAlpha = _calculateMinimumAlpha(
        white,
        color,
        _minContrastTitleText,
      );
      ...
      final int? darkBodyAlpha = _calculateMinimumAlpha(
        black,
        color,
        _minContrastBodyText,
      );
      final int? darkTitleAlpha = _calculateMinimumAlpha(
        black,
        color,
        _minContrastTitleText,
      );
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<Color>('titleTextColor', titleTextColor),
    );
...
/// Debug snapshot of the palette pipeline for parity analysis.
class PaletteDebugSnapshot {
  const PaletteDebugSnapshot({
    required this.quantizedHistogram,
    required this.filteredHistogram,
    required this.paletteColors,
    required this.selectedSwatches,
    required this.splitTrace,
  });

  final List<PaletteColor> quantizedHistogram;
  final List<PaletteColor> filteredHistogram;
  final List<PaletteColor> paletteColors;
  final Map<PaletteTarget, PaletteColor> selectedSwatches;
  final List<_QuantizerSplitTraceEntry> splitTrace;
}

class _QuantizerDebugResult {
  const _QuantizerDebugResult({
    required this.quantizedHistogram,
    required this.filteredHistogram,
    required this.paletteColors,
    required this.splitTrace,
...
class _QuantizerSplitTraceEntry {
  const _QuantizerSplitTraceEntry({
    required this.step,
    required this.removedBox,
    required this.newBox,
    required this.remainingBox,
  });

  final int step;
  final _QuantizerBoxSnapshot removedBox;
  final _QuantizerBoxSnapshot newBox;
  final _QuantizerBoxSnapshot remainingBox;
}

class _QuantizerBoxSnapshot {
  const _QuantizerBoxSnapshot({
    ...
    required this.minGreen,
    required this.maxGreen,
    required this.minBlue,
...
enum _ColorComponent { red, green, blue }
...
class _ColorVolumeBox {
  ...
  _ColorVolumeBox splitBox() {
    ...
    final _ColorVolumeBox newBox = _ColorVolumeBox(
      splitPoint + 1,
      _upperIndex,
      histogram,
      colors,
  ...
  int _findSplitPoint() {
    ...
    // dimension.
    final List<Color> colorSubset = colors.sublist(
      _lowerIndex,
      _upperIndex + 1,
    );
  ...
  _QuantizerBoxSnapshot toDebugSnapshot() {
    return _QuantizerBoxSnapshot(
...
class _ColorCutQuantizer {
  ...
  Completer<_QuantizerDebugResult>? _paletteDebugCompleter;
  FutureOr<_QuantizerDebugResult> get quantizedColorsWithDebug async {
    if (_paletteDebugCompleter == null) {
      _paletteDebugCompleter = Completer<_QuantizerDebugResult>();
      _paletteDebugCompleter!.complete(_quantizeColorsWithDebug());
    }
    return _paletteDebugCompleter!.future;
  }
  ...
  List<PaletteColor> _quantizeColors() {
    return _quantizeColorsWithDebug().paletteColors;
  }

  _QuantizerDebugResult _quantizeColorsWithDebug() {
    ...
    final List<PaletteColor> quantizedHistogram = <PaletteColor>[
      for (final Color color in hist.keys)
        PaletteColor(color, hist[color]!.value),
    ];
    ...
    if (hist.length <= maxColors) {
      ...
      paletteColors.clear();
      final _QuantizePixelsDebugResult quantizeDebug = _quantizePixels(
        maxColors,
        hist,
      );
      paletteColors.addAll(quantizeDebug.paletteColors);
      return _QuantizerDebugResult(
        quantizedHistogram: quantizedHistogram,
        filteredHistogram: <PaletteColor>[
          for (final Color color in hist.keys)
            PaletteColor(color, hist[color]!.value),
        ],
        paletteColors: paletteColors,
        splitTrace: quantizeDebug.splitTrace,
      );
    }
    return _QuantizerDebugResult(
      quantizedHistogram: quantizedHistogram,
      ...
      paletteColors: paletteColors,
      splitTrace: const <_QuantizerSplitTraceEntry>[],
    );
  }

  _QuantizePixelsDebugResult _quantizePixels(
    int maxColors,
    ...
    priorityQueue.add(
      _ColorVolumeBox(
        ...
        histogram.length - 1,
        histogram,
        histogram.keys.toList(),
      ),
    );
    // Now go through the boxes, splitting them until we have reached maxColors
    // or there are no more boxes to split
    final List<_QuantizerSplitTraceEntry> splitTrace = _splitBoxes(
      priorityQueue,
      maxColors,
    );
    // Finally, return the average colors of the color boxes.
    return _QuantizePixelsDebugResult(
      paletteColors: _generateAverageColors(priorityQueue),
  ...
  List<_QuantizerSplitTraceEntry> _splitBoxes(
    PriorityQueue<_ColorVolumeBox> queue,
    final int maxSize,
  ) {
    final List<_QuantizerSplitTraceEntry> trace = <_QuantizerSplitTraceEntry>[];
    int step = 0;
    while (queue.length < maxSize) {
      final _ColorVolumeBox colorVolumeBox = queue.removeFirst();
      if (colorVolumeBox.canSplit()) {
        // First split the box, and offer the result
        final _QuantizerBoxSnapshot removedBox = colorVolumeBox
            .toDebugSnapshot();
        final _ColorVolumeBox newBox = colorVolumeBox.splitBox();
        queue.add(newBox);
        // Then offer the box back
        queue.add(colorVolumeBox);
        trace.add(
          _QuantizerSplitTraceEntry(
            step: step++,
            removedBox: removedBox,
            newBox: newBox.toDebugSnapshot(),
            remainingBox: colorVolumeBox.toDebugSnapshot(),
          ),
        );
      } else {
...
class _QuantizePixelsDebugResult {
  const _QuantizePixelsDebugResult({
    required this.paletteColors,
    required this.splitTrace,
  });
